...
4. **GPU Type:** Select **NVIDIA A10 (24 GB)** for the best price/performance
   - *For budget demos, a fractional **A40 (8 GB VRAM)** works with `--fractional` mode (see below).*
...
### Fractional GPU Deployment (8 GB VRAM slice)
If you're using a Vultr fractional A40 instance (`vcg-a40-4c-20g-8vram`):

```bash
./deploy_to_vultr.sh --fractional 203.0.113.45
```

This uses `docker-compose.fractional.yml`, which constrains Ollama to:
- `OLLAMA_NUM_PARALLEL=1`
- `OLLAMA_MAX_LOADED_MODELS=1`
- `NUM_CTX=4096` / `NUM_PREDICT=1024`
- `UVICORN_WORKERS=1`

> ⚠️ **Concurrent users warning:** A fractional GPU is suitable for **single-user demos only**. Multiple parallel requests can cause OOM errors.
...
# Restart everything (use the correct compose file)
sudo docker compose -f /opt/nzlegal/docker-compose.vultr.yml restart
# or for fractional:
sudo docker compose -f /opt/nzlegal/docker-compose.fractional.yml restart

# For fractional deployments:
sudo docker compose -f /opt/nzlegal/docker-compose.fractional.yml restart
...
| `docker-compose.vultr.yml` | Production Docker Compose for Vultr (full GPU) |
| `docker-compose.fractional.yml` | Constrained compose for 8 GB fractional GPU slices |
