        ...
        try:
            with zipfile.ZipFile(io.BytesIO(zip_content)) as zf:
                for info in zf.infolist():
                    ...
                    ext = archive_obj.suffix.lower()
                    if ext not in self.SUPPORTED_TYPES:
                        continue
                    
                    try:
                        content = zf.read(info.filename)
                        doc = self.parse_file(content, archive_obj.name)

                        if getattr(doc, "metadata", None) is None:
                            doc.metadata = {}

                        parts = [part for part in archive_obj.parts if part not in ("", ".")]
                        top_level_folder = parts[0] if len(parts) > 1 else ""

                        doc.metadata["archive_path"] = archive_path
                        doc.metadata["archive_name"] = archive_obj.name
                        doc.metadata["top_level_folder"] = top_level_folder
