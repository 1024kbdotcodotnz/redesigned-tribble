...
# Usage: ./deploy_to_vultr.sh [--fractional] <VULTR_IP> [SSH_USER]

FRACTIONAL=false
if [ "${1:-}" = "--fractional" ]; then
    FRACTIONAL=true
    shift
fi
...
if [ -z "$VULTR_IP" ]; then
    echo "Usage: ./deploy_to_vultr.sh [--fractional] <VULTR_IP> [SSH_USER]"
    echo ""
    echo "Options:"
    echo "  --fractional   Use docker-compose.fractional.yml (for 8GB VRAM slices)"
    echo ""
    echo "Examples:"
    echo "  ./deploy_to_vultr.sh 203.0.113.45"
    echo "  ./deploy_to_vultr.sh --fractional 203.0.113.45"
    exit 1
fi

if [ "$FRACTIONAL" = true ]; then
    COMPOSE_FILE="docker-compose.fractional.yml"
    echo "🎯 Fractional GPU mode selected ($COMPOSE_FILE)"
else
...
tar -chzf "$TARBALL" \
    ...
    --exclude='chroma_db.bak' \
    --exclude='nz_legal_rag_deploy' \
    --exclude='nz_legal_rag_deploy/' \
    -C "$(dirname "$0")" \
    .
...
ssh "$SSH_USER@$VULTR_IP" << EOF
    ...
    if [ "$FRACTIONAL" = true ]; then
        touch .fractional
        echo "Fractional mode marker created (.fractional)"
    else
        rm -f .fractional
    fi
