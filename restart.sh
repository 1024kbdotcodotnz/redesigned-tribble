...
# Configure Ollama for full GPU offload on RTX 3090
...
# Stop any systemd-managed Ollama so it doesn't fight with our server
systemctl stop ollama 2>/dev/null || true
systemctl disable ollama 2>/dev/null || true

# Start Ollama server
echo "Starting Ollama server with GPU offload..."
...
for model in "${REQUIRED_MODELS[@]}"; do
    if curl -s localhost:$OLLAMA_PORT/api/tags | grep -q "\"name\":\"$model\""; then
        ...
        echo "  Pulling $model..."
        curl -s -X POST localhost:$OLLAMA_PORT/api/pull \
...
# Pre-load the LLM and verify it is 100% GPU before starting the API
echo "Pre-loading $LLM_MODEL onto GPU..."
curl -s -X POST "localhost:$OLLAMA_PORT/api/generate" \
    -H "Content-Type: application/json" \
...
for i in $(seq 1 30); do
    ...
    if echo "$PS_OUT" | grep -q "$LLM_MODEL"; then
        if echo "$PS_OUT" | grep "$LLM_MODEL" | grep -q "100% GPU"; then
            ...
            echo -e "  ${YELLOW}⚠${NC} Model is using CPU; retrying ($i/30)..."
...
if [ "$GPU_OK" = true ]; then
    echo -e "  ${GREEN}✓${NC} $LLM_MODEL loaded 100% on GPU"
else
    echo -e "  ${RED}⚠${NC} Could not confirm 100% GPU offload. Check logs/ollama.log for CUDA/Vulkan messages."
    echo "    Continuing anyway, but generation may be slow."
fi
