#!/bin/bash
# NZ Legal RAG - Full Shutdown & Restart Script
...
kill_port() {
    ...
    if [ -z "$pid" ]; then
        pid=$(ss -tlnp 2>/dev/null | grep ":$port " | awk '{print $7}' | cut -d'=' -f2 | cut -d',' -f1)
    fi
    if [ -n "$pid" ] && [ "$pid" != "-" ]; then
...
if curl -s localhost:$API_PORT/health > /dev/null 2>&1; then
    HEALTH_STATUS=$(curl -s localhost:$API_PORT/health | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('database',{}).get('total_documents',0))" 2>/dev/null || echo "unknown")
    echo -e "  ${GREEN}✓${NC} API healthy - $HEALTH_STATUS documents loaded"
else
    echo -e "  ${RED}✗${NC} API not responding"
fi

if curl -s localhost:$OLLAMA_PORT/api/tags > /dev/null 2>&1; then
    MODEL_COUNT=$(curl -s localhost:$OLLAMA_PORT/api/tags | grep -o '"name"' | wc -l)
